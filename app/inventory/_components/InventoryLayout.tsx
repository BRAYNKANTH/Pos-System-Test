  ...
  return (
    <div className="min-h-screen bg-zinc-100 text-zinc-900 flex print:bg-white print:text-black">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Top Navbar Header */}
        <header className="h-16 bg-indigo-900 text-white px-3 sm:px-5 flex items-center justify-between gap-2 border-b shadow-sm shrink-0 sticky top-0 z-30 print:hidden">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <span className="font-bold text-base lg:hidden truncate min-w-0">{businessName}</span>
            <div className="hidden lg:flex items-center gap-2 text-sm font-semibold min-w-0">
              <span className="text-zinc-300 shrink-0">Location:</span>
              <span className="bg-white/10 px-2.5 py-1 rounded text-white text-xs truncate">{businessName}</span>
            </div>
            <div className="hidden sm:block bg-white/10 px-2.5 py-1 rounded text-white text-xs font-bold font-mono shrink-0">
              {currentDate}
            </div>
          </div>

          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <button className="hidden sm:flex h-8 w-8 hover:bg-white/10 rounded items-center justify-center text-zinc-300 transition">
              <Plus className="h-4.5 w-4.5" />
            </button>
            <Link
              href="/pos"
              className="bg-green-600 hover:bg-green-700 text-white px-3.5 py-1.5 rounded-md text-xs font-bold flex items-center gap-1.5 shadow-sm transition shrink-0"
            >
              <Grid className="h-4 w-4" />
              <span className="hidden sm:inline">POS</span>
            </Link>
            <button className="hidden sm:flex h-8 w-8 hover:bg-white/10 rounded items-center justify-center text-zinc-300 transition relative">
